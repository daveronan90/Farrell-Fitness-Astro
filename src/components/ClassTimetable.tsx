...
import { Cell, Row, defaultClasses } from '../js/classesData'
import { getGoogleSheetsData } from '../js/googleSheets'

const Cell = React.memo(({ cell, cIdx, rIdx }: { cell: Cell; cIdx: number; rIdx: number }) => (
	<div
		key={cIdx}
		className={`flex items-center justify-center px-4 py-2 md:py-4 ${
			cIdx === 0 && 'text-xxs text-primary md:text-base '
		}
            ${rIdx === 0 && cIdx !== 0 && 'border-primary bg-primary text-xxs md:text-base'}
...
const RowComponent = React.memo(({ row, rIdx }: { row: Row; rIdx: number }) => (
	<div key={rIdx} className={`grid grid-cols-8 gap-[0.1rem] divide-x-2 divide-y-2`}>
		{row.map((cell, cIdx) => (
	...
	return (
		...
		>
			<div className="mb-3 flex flex-col space-y-1 text-2xl md:mb-6 md:text-3xl">
				...
				<h2 className="text-xs text-secondary-500 md:text-base">may 29 to 3 september</h2>
			</div>
